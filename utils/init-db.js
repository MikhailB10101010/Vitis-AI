...
try {
  ...
  db.exec(`CREATE INDEX IF NOT EXISTS idx_evaluations_created_at ON evaluations(created_at DESC)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_evaluations_suitability_score ON evaluations(suitability_score DESC)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_evaluations_category ON evaluations(category)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_evaluations_cached ON evaluations(cached, cache_expiry)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_evaluations_status ON evaluations(status)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_evaluations_region ON evaluations(region)`);
  
  // Composite index for user's evaluations
  db.exec(`CREATE INDEX IF NOT EXISTS idx_evaluations_user_created ON evaluations(user_id, created_at DESC)`);
  
  // Index for geospatial queries (using latitude/longitude)
  db.exec(`CREATE INDEX IF NOT EXISTS idx_evaluations_coords ON evaluations(geo_latitude, geo_longitude)`);
